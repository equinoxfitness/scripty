...
pipeline{
    ...
    environment {
        CODACY_PROJECT_TOKEN = credentials('codacy-scripty')
    }
    triggers {
        pollSCM 'H/10 * * * *'
    }
    ...
    stages{
        stage('Coverage to Codacy'){
            steps {
                slackSend (color: 'good', message: "scripty_pypi_pipeline_${GIT_BRANCH} - Starting build #${BUILD_NUMBER}. (<${env.BUILD_URL}|Open>)")
    ...
    post {
        failure {
            echo "fail"
            slackSend (color: 'danger', message: "@here scripty_pypi_pipeline_${GIT_BRANCH} - Build #${BUILD_NUMBER} Failed. (<${env.BUILD_URL}|Open>)")
        }
        success {
            echo "good"
            slackSend (color: 'good', message: "scripty_pypi_pipeline_${GIT_BRANCH} - Build #${BUILD_NUMBER} Success. (<${env.BUILD_URL}|Open>)")
        }
        always {
